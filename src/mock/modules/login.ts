...
const signIn = {
  url: '/login/signIn', //  或者需要拦截请求的完整url `${api}/login/kaptcha`
  method: 'post',
  template: {
    message: '登录成功',
    code: 200,
    data: {
      ...
      token: 'login_token_daytoywhyTodayisgood'
    }
  }
}

const signOut = {
  url: '/login/signOut', //  或者需要拦截请求的完整url `${api}/login/kaptcha`
  method: 'get',
  template: {
    message: '退出登录',
    code: 200,
...
export default [kaptcha, signIn, signOut]